...
class core_badges_observer {
    ...
    /**
     * Triggered when 'achievement_reached' event happens.
     *
     * @param \block_game_achievements\event\achievement_reached $event event generated when an achievement is reached.
     */
    public static function achievement_criteria_review(\block_game_achievements\event\achievement_reached $event) {
        global $DB, $CFG;

        if (!empty($CFG->enablebadges)) {
            require_once($CFG->dirroot.'/lib/badgeslib.php');

            $achievementids = $event->other['achievementids'];
            $achievementuserids = $event->other['achievementuserids'];

            foreach($achievementids as $i => $achievementid) {
                // Need to take into account that there can be more than one badge with the same achievement in its criteria.
                if($rs = $DB->get_records('badge_criteria_param', array('name' => 'achievement_' . $achievementid, 'value' => $achievementid))) {
                    foreach($rs as $r) {
                        $bid = $DB->get_field('badge_criteria', 'badgeid', array('id' => $r->critid), MUST_EXIST);
                        $badge = new badge($bid);
                        ...
                        foreach($achievementuserids[$i] as $userid) {
                            if($badge->is_issued($userid)) {
                                continue;
                            }

                            if($badge->criteria[BADGE_CRITERIA_TYPE_ACHIEVEMENT]->review($userid)) {
                                $badge->criteria[BADGE_CRITERIA_TYPE_ACHIEVEMENT]->mark_complete($userid);

...
class block_badges extends block_base {
    ...
    public function get_content() {
        ...
        $this->content->text = '<p><b>' . get_string('mybadges', 'block_badges') . '</b><br>'; 
        if ($user_badges = badges_get_user_badges($USER->id, $courseid, 0, $this->config->numberofbadges)) {
            $this->content->text .= $output->print_badges_list($user_badges, $USER->id, true);
        } else {
            $this->content->text .= get_string('nothingtodisplay', 'block_badges');
        }
        $this->content->text .= '</p>';

        $badges = badges_get_badges(BADGE_TYPE_SITE, 0, 'name', 'ASC') + badges_get_badges(BADGE_TYPE_COURSE, $courseid, 'name', 'ASC');
        foreach ($user_badges as $user_badge) {
            unset($badges[$user_badge->id]);
        }
        if (!empty($badges)) {
            $this->content->text .= '<p><b>' . get_string('badgesicanearn', 'block_badges') . '</b><ul>'; 
            foreach ($badges as $badge) {
                if($badge->status == BADGE_STATUS_ACTIVE || $badge->status == BADGE_STATUS_ACTIVE_LOCKED) {
                    $this->content->text .= '<li>' . $badge->name . '<br>' . $output->print_badge_criteria($badge, 'short') . '</li>';
                }
            }
